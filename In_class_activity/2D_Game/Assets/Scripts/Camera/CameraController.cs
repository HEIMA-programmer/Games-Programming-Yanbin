...
public class CameraController : MonoBehaviour
{
    ...
    /// void (no return)
    /// </summary>
    // Optional shake component on the same GameObject. Looked up once in Start.
    ...
    private void SetCameraPosition()
    {
        if (target != null)
        {
            ...
            // Lazy-find the shake component (works whether or not one is attached)
            if (cameraShake == null) cameraShake = GetComponent<CameraShake>();
            Vector3 shakeOffset = cameraShake != null ? cameraShake.GetCurrentOffset() : Vector3.zero;
