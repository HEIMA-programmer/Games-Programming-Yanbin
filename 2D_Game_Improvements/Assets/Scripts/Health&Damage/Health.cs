...
public class Health : MonoBehaviour
{
    ...
    {
        // Truly invincible (e.g., walls) — no feedback at all
        if (isAlwaysInvincible)
        {
            return;
        }

        // Polish: ALWAYS give visual feedback so the player knows they were hit,
        // even during invincibility frames (just no damage applied)
        ...
        if (gameObject.tag == "Player" && CameraShake.instance != null)
        {
            CameraShake.instance.Shake();
        }

        // Damage is gated by invincibility frames
