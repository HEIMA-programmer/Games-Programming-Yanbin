...
line-of-sight cone and a gaze-alarm meter, checkpoints, and one collectible memory
fragment per level.
...
respawning at a checkpoint. Low dexterity floor — multiple key bindings, no
simultaneous-press requirements, chase speeds tuned below run speed, generous
...
- **The real slice (v0.8.0, 10 June):** Level 0 ("Awakening") hand-authored over the
  ...
  contract. Acceptance: a new player reaches the exit unaided, every mechanic the game
  ships is demonstrated at least once, no debug input exists in the build.
...
- **Procedural level generation** — retired by design on 10 June: shipped scenes are
  hand-authored; the generators are fenced behind a `Legacy (DANGER)` editor menu.
...
- **Pipeline strategy (it evolved, deliberately):** Phase 1 — everything (sprites, SFX,
  ...
  1-bit art baseline + tilemaps. Phase 3 — hand-authored scenes as the single source of
  truth, with the generators retired. Each shift is logged in the DevLog with reasons.
- **Process:** public GitHub repo, one PR per feature with a playable build each merge
  (#1–#156), playtest rounds gating milestones, licence check before any asset enters
  the repo.
- **Testing:** in-editor play passes per PR; scripted editor probes for regressions
  (e.g. verifying plate latching, mine/crate immunity, beat triggers); five recorded
  playtest rounds with fix lists ([PlayTestNotes/](PlayTestNotes/)).
...
| Plates & doors | `PressurePlate`, `Door`, `PlateActivator`, `AutoDoorExit` | weight detection (player/echo/crate), multi-plate doors with `requireAll` + latch, exit doors with audio |
...
| UI | `HUDController`, `MainMenuController`, `PauseMenu`, `VictoryScreen`, `LevelButton`, `ButtonSfx` | fragment counter, record key hint, menus with the dimensional title |
...
| Editor tooling | `EchoArt`, `EchoAudio`, `EchoUI`, `EchoCutscenes`, `EchoBuildUtils` | generators for the procedural sprites/SFX still shipped, gameplay-UI and cutscene-scene builders — the provenance of every non-imported asset |
...
- **CraftPix 1-bit kit** (player, tiles, props, drone — royalty-free, but source-file
  ...
  game README). Tinting is done at runtime (`SpriteRenderer.color`) so no derived
...
- **Accessibility:** multiple bindings per action and no simultaneous-press chords;
  every important event is double-coded (visual + audio: plate light + click, drone
  beam brightness + alarm meter, record vignette + hum); no harsh strobing — hazards
  telegraph on slow alpha pulses; story beats and cutscenes are skippable; failure is a
  checkpoint respawn, never lost progress; 1-bit palette keeps contrast maximal.
...
| W4 | 9–15 Jun | M3 — content, narrative & ship | Three levels hand-rebuilt (#150–#152); Playtest 04; **Level 3 cut**; four cutscene acts + soundtrack = **v1.0.0** (#153); presentation polish + licence compliance (v1.1.x, #155–#156) |
| W5 | 16–21 Jun | Submission & presentation | Documentation set (this rewrite), final validation pass, presentation and oral-exam prep |
