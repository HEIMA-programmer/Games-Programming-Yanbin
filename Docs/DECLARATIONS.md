...
6. **What I used unchanged:** the pixel art itself (no repainted copies exist on disk).
7. **What I modified:** frame selection and grid slicing (Unity import settings), runtime tinting to the 1-bit palette (white/cyan via `SpriteRenderer.color`), composition into tilemaps.
8. **What I created myself:** every level layout, tilemap composition, collision setup and animation timing that uses the kit.
9. **Where it appears in my game:** all three levels and the menu — characters, terrain, doors, checkpoints, items, props, HUD frame elements.
...
6. **Orbitron** — Matt McInerney, via Google Fonts. Appears: fallback display font.
...
8. **"Slow Melancholic Theme (C64 Style)"** — skrjablin — CC0 1.0 — menu + opening act. <https://opengameart.org/content/slow-melancholic-theme-c64-style>
...
**No other external assets, templates, tutorial code or code snippets were used.** All
sound effects are synthesised by the project's own editor scripts; all sprites outside
A1–A3 and A13 are generated by the project's own tooling.
...
specification.** It works as an agentic assistant that can edit project files directly
under my supervision; I reviewed the resulting changes, tested them in the editor, and
made every commit, merge and release myself.

1. **Tool used:** Claude Code (Anthropic), used in supervised working sessions from
   25 May to June 2026. (A separate AI image tool was used only for the cutscene
   illustrations — declared in §A13.)
...
5. **How I tested it:** every change was played in the editor before it counted — a
   ...
   Five recorded playtest rounds (three with an external coursemate tester) produced
   ...
   confirmed by reopening the saved scene from disk. Puzzle invariants were verified
   with scripted in-editor probes — doors latching open, mines ignoring crates, the
   fragment reachable from the crate top but provably not from the ground, story
   beats firing once and only on their intended condition. Nothing was committed
   untested, and every commit and merge is mine.
...
7. **What I still do not fully understand:** the internals I use through configuration
   ...
   ear); and how URP's 2D volumetric lights are rendered behind the drone
   searchlights. For each of these I can explain what it does, why it is in the
   project, and how it is configured — but not their inner implementation.
